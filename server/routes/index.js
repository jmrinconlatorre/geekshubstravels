...
router.post('/users', (req, res) => {//ESTO ES API REST (los demas son handlebars, no todos)
    new User(req.body)
        ...
        .then(user => { //nos devuelve el usuario que ha guardado en la respuesta
            ...
            res.status(400).json(err);//lo hemos cambiado a json porque send devuelve texto plano (juan nos lo ha dicho asi)
        });
});
